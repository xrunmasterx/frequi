...
const tableColumns = computed(() => [
  {
    ...
    header: t('webserver.analysis.indicator'),
...
<template>
  <div class="flex flex-col gap-2">
    <div class="flex flex-wrap gap-x-6 gap-y-1">
      <span
        ><span class="font-bold">{{ t('webserver.analysis.strategy') }}</span>
        {{ result.strategy }}</span
      >
      <span>
        <span class="font-bold">{{ t('webserver.analysis.recommendedStartupCandleCount') }}</span>
        {{ result.strategy_scc }}
      </span>
    </div>
    ...
    <UAlert
      ...
      :title="t('webserver.analysis.noRecursiveIssues')"
      :description="t('webserver.analysis.noRecursiveIssuesDescription')"
    />
    ...
    <div v-else class="overflow-x-auto">
      <UAlert
        ...
        icon="i-mdi-alert"
        :title="
          formatLocaleText(t('webserver.analysis.affectedStartupCandleCount'), {
            count: indicators.length,
          })
        "
        :description="t('webserver.analysis.affectedStartupCandleCountDescription')"
      />
